...
export default function Organization() {
  ...
  const getManagerTypeIcon = (managerType?: "truong" | "pho" | "canbo") => {
    switch (managerType) {
      case "truong":
        return "👑"
      case "pho":
  ...
  const handleDelete = (id: string) => {
    if (confirm("Bạn có chắc chắn muốn xóa đơn vị này? Tất cả đơn vị con cũng sẽ bị xóa.")) {
      const deleteRecursive = (deptId: string, currentDepartments: Department[]): Department[] => {
        const children = currentDepartments.filter((d) => d.parentId === deptId)
        let remainingDepartments = currentDepartments.filter((d) => d.id !== deptId)
        for (const child of children) {
          remainingDepartments = deleteRecursive(child.id, remainingDepartments)
        }
        return remainingDepartments
      }

      const updatedDepartments = deleteRecursive(id, departments)
      saveDepartments(updatedDepartments)
    }
  }
  ...
  const renderDepartmentNode = (dept: any, level = 0) => {
    ...
    const sortedOfficers = deptOfficers.sort((a, b) => {
      ...
      return (order[a.managerType as keyof typeof order] || 3) - (order[b.managerType as keyof typeof order] || 3)
    })
              ...
              <div className="space-y-2 pl-9">
                {sortedOfficers.map((officer) => (
                  <div key={officer.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg border">
                    <div className="flex items-center space-x-3">
                      <span className="text-lg">{getManagerTypeIcon(officer.managerType)}</span>
                      <div>
                        <div className="flex items-center space-x-2">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    <SelectItem value={null as any}>Không có (Cấp cao nhất)</SelectItem>
                    {departments
